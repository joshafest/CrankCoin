...
unsigned int nTargetSpacing = 50;
unsigned int nStakeMinAge = 50 * 60;
unsigned int nStakeMaxAge = 5 * 24 * 60 * 60;
...
const string strMessageMagic = "CrankCoin Signed Message:\n";
...
if (pindexBest->nHeight+1 > 2) { nSubsidy = (2*sin(pindexBest->nHeight* (pie/120) ) + 2) * COIN; return nSubsidy; }
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    if (!CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
        ...
        if (CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
        printf("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, modifier=0x%016"PRIx64, pindexNew->nHeight, pindexNew->nStakeModifierChecksum);
...
bool CBlock::AcceptBlock()
{
    ...
    if ((nBits != GetNextTargetRequired(pindexPrev, IsProofOfStake()))&& pindexPrev->nHeight >= 5618)
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "CrankCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Problems in General Physics written by Irodov is best book ever.";
        CTransaction txNew;
        txNew.nTime = 1450449893;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
		...
		uint256 nhashMerkleRoot = uint256("0x04af182d9f8ecb33a52c980d9b2e02e7214484a8b8be54513e54d3ce75f8b906");
